...
const Feed = () => {
  ...
  const feedItems = useMemo(() => {
    ...
    const timelineItems = [...timelineEvents].map((event) => {
      ...
      const location = incident ? `${incident.lat.toFixed(4)}°, ${incident.lng.toFixed(4)}°` : 'Unknown location'
      return {
        ...
        reporter: incident ? `Incident ${incident.incidentId}` : 'System',
      ...
      <div className="space-y-8 md:space-y-12">
        {feedItems.length === 0 ? (
          <div className="rounded-3xl border border-outline/20 bg-white/80 p-10 text-center text-espresso/60">
            No live distress signals available.
          </div>
        ) : (
                ...
                <div className="w-full md:w-1/4 flex flex-row md:flex-col justify-between border-b md:border-b-0 md:border-r border-outline/20 pb-4 md:pb-0 md:pr-8">
                  <div>
                    <div className={`inline-block px-2 md:px-3 py-0.5 md:py-1 mb-4 md:mb-6 text-[9px] md:text-[10px] font-black tracking-[.2em] border ${
                      signal.status === 'PENDING' ? 'bg-terracotta text-white border-terracotta' :
